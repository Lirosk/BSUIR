data = __import__("БАСЕНКО КИРИЛЛ АЛЕКСАНДРОВИЧ БЕЛОРУССКИЙ ГОСУДАРСТВЕННЫЙ УНИВЕРСИТЕТ ИНФОРМАТИКИ И РАДИОЭЛЕКТРОНИКИ КСИС ИИТП 953505 МАТЕМАТИКА ЧИСЛЕННЫЙ АНАЛИЗ ЛР 4 РЕШЕНИЕ СИСТЕМ НЕЛИНЕЙНЫХ УРАВНЕНИЙ МИНСК 21 data.py Python 3.8.2 32-bit")
...
def nm(x0: list) -> None:
    ...
    while (iters < lim) and not (max([abs(x[0] - pre_x[0]), abs(x[1] - pre_x[1])]) < data.fr):
        ...
        x[0] = float(pre_x[0] - data.f1(pre_x)*(data.j[0].subs({data.abc.x: pre_x[0], data.abc.y: pre_x[1]}) + data.j[1].subs({data.abc.x: pre_x[0], data.abc.y: pre_x[1]})))
        x[1] = float(pre_x[1] - data.f2(pre_x)*(data.j[2].subs({data.abc.x: pre_x[0], data.abc.y: pre_x[1]}) + data.j[3].subs({data.abc.x: pre_x[0], data.abc.y: pre_x[1]})))
