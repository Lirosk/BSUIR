...
def encrypt(key: str, obj: str | tuple | int) -> str:
    if (type(obj) is tuple):
        return tuple(encrypt(key, item) for item in obj)
    obj = str(obj)

    remainder = 8 - len(obj) % 8
    if (remainder != 8):
        obj = " " * remainder + obj

    res = des.encode(obj, key)

    return res

def decrypt(key: str, obj: str | tuple) -> str:
    if (type(obj) is tuple):
        return tuple(decrypt(key, item) for item in obj)
    
    res = des.decode(obj, key)

    return res.strip()

def gen_lengthes(obj: tuple | str):
    if (type(obj) is tuple):
        return tuple(gen_lengthes(item) for item in obj)
